...
def game():
    ...
    while not game_over:
        ...
        reveal_cell(row, col, bomb_grid, show_grid)
...
def handle_flag(user_input, show_grid):
    # Get the coordinate part (remove "flag " from the input)
    flag_part = user_input[5:]  # This gets everything after "flag "
...
def reveal_cell(row, col, bomb_grid, show_grid):
        ...
        for dx, dy in directions:
            ...
            if 0 <= new_row < 8 and 0 <= new_col < 8:
                if show_grid[new_row][new_col] == "⬜️":
                    reveal_cell(new_row, new_col, bomb_grid, show_grid)
